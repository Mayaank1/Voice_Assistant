...
def voice(assname):
    ...
    if 'wikipedia' in query:
        ...
        ans = Tk()
        ans.geometry("1366x768")
        T = Text(ans, height=10, width=70)
        T.pack()
        ...
        speak("I am your virtual assistant Digi Bud")
        ...
        print(" Temperature (in kelvin unit) = " + str(
            ...
            weather_description))
        results=f"Current Temperature is {current_temperature},Current Humidity is {current_humidity},current pressure is {current_pressure}and description is {weather_description}"
        speak(results)
        ans = Tk()
        ans.geometry("1366x768")
        T = Text(ans, height=10, width=70)
        K=Text(ans,height=3,width=30)
        K.pack()
        T.pack()
        a="Weather in"+city_name
        K.insert(END,a)
        T.insert(END, results)
        ...
        try:
            jsonObj = urlopen(
                '''https://newsapi.org/v2/top-headlines?country=in&apikey=5fcb732b78b34006a282c1a0f218d8e0''')
...
while True:
    ...
    master.title("DIGI BUD")
    border=Frame(master,highlightbackground="white",highlightthickness=4).pack(padx=20,pady=20)
    img1= ImageTk.PhotoImage(Image.open("Capture.png").resize((200,200)))
    Label(master, text=" DIGI BUD " , font = ('calibri',20)).pack()
    Label(master,image=img1).pack()
    Button(master, text="Start", command=greeting,height=2,width=6).place(x=700,y=500)
    Button(master, text="Ask Me", command=lambda:voice(assname), height=2, width=6).place(x=700, y=550)
    Button(master, text="Stop", command=end_pro, height=2, width=6).place(x=700, y=600)
    master.mainloop()